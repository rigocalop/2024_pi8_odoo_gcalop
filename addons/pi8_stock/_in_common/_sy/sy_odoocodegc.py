...
class sy_OdooCodeGC:
    ...
    def joinLot_ModeTextCode(cls, env, target_list, field_lot=['lot'], model_fields=['id', 'name', 'product_id'], mapping_fields={'lot_id': 'id', 'product_id': 'product_id[0]'}):
        list_values = sx.XListDict.SelectDistinct(target_list, fields=field_lot, remove_empty=True)
        reference_data = sy_OdooModel.SearchIn(env, model_name='stock.lot', fields=model_fields, search_field='name', search_values=list_values)
        target_list = sx.XListDict.join(target_list, target_field_on=field_lot, reference_data=reference_data, reference_field_on='name', mapping_fields=mapping_fields)
        return target_list
    
    @classmethod
    @hlog_function()
    def joinLot_ModeID(cls, env, target_list, field_lot='lot_id', model_fields=['name', 'product_id'], mapping_fields={'lot': 'name', 'product_id': 'product_id[0]'}):
    ...
    @classmethod
    @hlog_function()
    def joinCode_ModeID(cls, env, target_list, field_id='product_id', model_fields=['id', 'default_code'], mapping_fields={'code': 'default_code'}):
        # Actualizar información desde 'stock.lot' para los casos en que no se encuentre el lot_name
    ...
    def EnsureCodeLot(cls, env, textcodes):
        ...
        # # Actualizar la información de productos.. y agregar parametro de tipo de retorno. return list_CodeLot.
        # if is_return_list:
        #     # list_CodeLot = cls.joinCode_ModeID(env=env, target_list=list_CodeLot)
        #     # list_CodeLot = cls.joinLot_ModeID(env=env, target_list=list_CodeLot)
        #     return True, list_CodeLot
        # else:
        #     return True, None
